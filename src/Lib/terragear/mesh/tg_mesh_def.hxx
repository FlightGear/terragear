...
// we are going to use a custom kernel for the arrangement
// it will be based on EPECK.  We will create a new point class that can be 
// constrained to a tile edge - useful during clustering and snap rounding.

#include <CGAL/Cartesian.h>

// terragear custom kernel
#include <terragear/kernels/tg_kernel.h>
...
typedef TgKernel<EPECK::FT>                                       MK;
typedef CGAL::Filtered_kernel_adaptor<MK>                         meshArrKernel;
...
typedef meshArrTraits::Vector_2                                   meshArrVector;
typedef meshArrTraits::Line_2                                     meshArrLine;
typedef CGAL::Polygon_2<meshArrKernel>                            meshArrPolygon;
typedef CGAL::Arrangement_2<meshArrTraits>                        meshArrangement;
typedef meshArrangement::Face_iterator                            meshArrFaceIterator;
typedef meshArrangement::Face_handle                              meshArrFaceHandle;
typedef meshArrangement::Face_const_handle                        meshArrFaceConstHandle;
typedef meshArrangement::Halfedge_handle                          meshArrHalfedgeHandle;
typedef meshArrangement::Halfedge_const_handle                    meshArrHalfedgeConstHandle;
typedef meshArrangement::Ccb_halfedge_circulator                  meshArrHalfedgeCirculator;
typedef meshArrangement::Ccb_halfedge_const_circulator            meshArrHalfedgeConstCirculator;
typedef meshArrangement::Halfedge_around_vertex_circulator        meshArrIncidentHalfedgeCirculator;
typedef meshArrangement::Halfedge_around_vertex_const_circulator  meshArrIncidentHalfedgeConstCirculator;
...
typedef meshTriKernel::Vector_2                                                             meshTriVector;
...
typedef meshTriCDT::Edge                                                                    meshTriEdge;
typedef meshTriCDT::Vertex_handle                                                           meshTriVertexHandle;
typedef meshTriCDT::Face_handle                                                             meshTriFaceHandle;
typedef meshTriCDT::Face_circulator                                                         meshTriFaceCirculator;
typedef meshTriCDT::Finite_faces_iterator                                                   meshTriFaceIterator;
...
typedef CGAL::Delaunay_mesher_2<meshTriCDT, meshCriteria>                                   meshRefinerWithEdgeModification;
typedef CGAL::Delaunay_mesher_no_edge_refinement_2<meshTriCDT, meshCriteria>                meshRefinerWithoutEdgeModification;
...
public:
    ...
    meshVertexInfo( OGRFeature* poFeature ) {
        vh        = meshTriTDS::Vertex_handle();

        int fieldIdx = poFeature->GetFieldIndex( "tg_vert" );
        if ( fieldIdx >= 0 ) {
            id = poFeature->GetFieldAsInteger(fieldIdx);
        } else {
            exit(0);
        }

        double x,y;
        fieldIdx = poFeature->GetFieldIndex( "tds_x" );
        if ( fieldIdx >= 0 ) {
            x = poFeature->GetFieldAsDouble(fieldIdx);
        } else {
            exit(0);
        }
        fieldIdx = poFeature->GetFieldIndex( "tds_y" );
        if ( fieldIdx >= 0 ) {
            y = poFeature->GetFieldAsDouble(fieldIdx);
        } else {
            exit(0);
        }
        pt        = meshTriPoint( x,y );

        fieldIdx = poFeature->GetFieldIndex( "tds_z" );
        if ( fieldIdx >= 0 ) {
            elevation = poFeature->GetFieldAsDouble(fieldIdx);
        } else {
            exit(0);
        }
    }
...
public:
    ...
    void getConstrainedField2( char* constrained ) const {
        sprintf(constrained, "%c %c %c", con[0]?'C':'N', con[1]?'C':'N', con[2]?'C':'N' );
    }
    ...
    void getVidxField2( char* indicies ) const {
        sprintf(indicies, "%d %d %d", vid[0], vid[1], vid[2] );
    }
    ...
    void getNIdxField2( char* indicies ) const {
        sprintf(indicies, "%d %d %d", nid[0], nid[1], nid[2] );
    }
