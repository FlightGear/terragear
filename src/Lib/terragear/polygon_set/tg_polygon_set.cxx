 ...
 *                                                                            *
 * vertex is degree 4                                                         *
 * result should be a boundary (A) and a hole (B)                             *
 *                                                                            *
 ...
 *                                                                            *
 * Traverse the CCB around the face - and handle the cases where we need to   *
 * 'diverge' off of the CCB to complete a simple polygon                      *
 * This got a bit more complicated, when looking at cs_drycrop in Europe      *
 * I've generated a valid test shapefile, and an algorithm to handle it.      *
 *                                                                            *
 * This is all handled in the tgPolygonSetPaths code.                         *
 *                                                                            *
 ******************************************************************************/
void tgPolygonSet::facesFromUntrustedNodes( 
    const std::vector<cgalPoly_Point>& nodes, 
    std::vector<cgalPoly_Polygon>& boundaries, 
    std::vector<cgalPoly_Polygon>& holes )
{
    ...
    // Create the path traversal helper to convert reletively simple polys to
    // simple polys.
    tgPolygonSetPaths paths( arr );
...
void tgPolygonSet::findIntersections( const cgalPoly_PolygonWithHoles& pwh, 
                                      const cgalPoly_Line& line, 
                                      std::vector<cgalPoly_Point>& intersections ) const
{
